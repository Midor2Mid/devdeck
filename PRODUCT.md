...
- **How do they solve it today?** Windows Terminal (tabs/panes) + VS Code + Postman/Insomnia + a database GUI, juggled manually — that part hasn't changed, and none of them knows about "the project I'm working on" as a single switchable context. What has changed, verified 2026-09-10: "none of them can tell me which agent needs an answer" was true when this line was written and is not true now. Claude Code's own CLI (`claude agents`, a needs-input-first list) and Desktop app (session sidebar, notifications) answer it for a Claude-only user with nothing beyond the CLI itself; Remote Control puts phone approve/deny on every plan, over Anthropic's own relay; `microsoft/intelligent-terminal` answers it inside a Windows Terminal fork over ACP; and Herdr — 37,000+ stars, Apache-2.0, a single Rust binary, no Electron, native Windows beta — badges every pane working/blocked/idle across a dozen-plus CLIs including `claude` and `codex`. That correction held for five days. **As of 2026-09-15 it is itself wrong**: `github.com/vc1492a/Pane` is AGPL-3.0, free, Electron, native Windows, agent-agnostic (Claude Code, Codex, Cursor Agent, Aider, Goose), worktree-per-agent, with a self-hosted Remote Pane for desktop or phone — every clause this line used to hold back (cross-vendor, the user's own Windows shell, no account, no relay, a GUI rather than a terminal multiplexer) held at once, free, today. Maestro, Nimbalyst and Munder Difflin sit in the same space, several on the same node-pty + xterm.js stack this app uses. What's still narrowly mine, as of today: the signal names its source, and refuses what it cannot source. An agent CLI can declare it is blocked over a hook instead of DevDeck inferring it from terminal output; a tile shows whether the agent said it or DevDeck guessed; a hook that cannot be matched to a session attributes nothing, rather than picking between two candidates. Nobody else does this — Herdr's own 0.9.0 release notes are a list of terminal-output cases it had to special-case, and Claude Code's own 2.1.269 changelog reads "fixed remote and headless sessions reporting 'waiting for your input' while background agents were still running": the vendor holding exact process state shipped a lying signal too. The honest caveat, undisguised: this survives *because nobody competes on it, which may also mean nobody buys on it.* The phone card has never rendered on real hardware, and D1 (`ROADMAP.md`) already deleted most of the panels that made "one window" mean something — so this is a narrower claim than the 09-10 one, which was itself narrower than the one before it.
...
- **What we offer:** One window where a **project** is the unit of context. Pick a project and every panel - terminals, editor, browser - snaps to it. Run many terminals and many agent sessions side by side, labeled and persistent, with one screen that shows every live agent across every project and which of them is waiting on you.
- **Why it's better than the status quo:** No more re-`cd`-ing and re-opening across four apps. Terminal-first layout (the terminal is a first-class citizen, not a drawer at the bottom). Agent-CLI affordances no terminal emulator has: one-click new session per project, per-session labels, and a supervision surface that names its source — Herdr and Pane will already tell you which agent needs you; this one also tells you whether that came from a hook the CLI sent or a guess from watching the terminal, and attributes nothing when a hook can't be matched to a session, rather than picking one.
