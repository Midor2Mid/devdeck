...
- **A test-count delta is only meaningful on a tree no other agent is touching.**
  Measured on `main` at 6425daa: **1,536 passed, 1 skipped**. During wave 1 the
...
**This is the one deletion with a keyboard consequence.** `Ctrl+Shift+B` is bound in `App.tsx` and advertised in `shortcuts.ts`, which is rendered by both the F1 overlay and Settings → Shortcuts. Removing the panel without removing the row leaves the reference lying — the exact defect fixed in `139bfc7`.
...
defect 139bfc7 just finished fixing."
```
