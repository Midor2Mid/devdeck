...
| 5 | A one-page homepage carrying SignPath's required attribution and the code-signing policy | `marketing` / `docs-writer` | The SignPath application, which requires a published policy | **survives, and it is now two clicks and one paragraph.** Updated 2026-09-15: the **mechanism exists** — `.github/workflows/pages.yml` publishes `site/` and only `site/` (`b7a73f3`), with `tests/pagesDeployScope.test.ts` failing if that artifact path ever widens, which is the guard that matters because everything else in this tree is internal. The hero is corrected: it is provenance-first and it names Herdr and Pane by name. **One thing stands, and it is no longer a document.** **(a)** Pages is still **not configured** (`gh api …/pages` → 404 on 2026-09-15) — an owner action, two clicks, gating nothing on this list. ~~**(b)** `site/index.html:250-251` still tells a reader the SignPath filing is impossible "because … this project's repository is still private."~~ **Closed later on 2026-09-15, and verified by reading the file rather than by being told:** the string is gone; the passage now says the filing has **not been applied for yet**, that the Foundation requires a public repository "which this one now is, since 2026-09-07", and that it simply has not been filed. That is the honest sentence — the obstacle named is the owner's inaction, not a fact about the repo. **So the last surviving instance of the falsehood class has been closed on the page it was worst on, and what remains of this row is two clicks by a person** |
...
| **10** | **The documents that still lie to a stranger** — `README.md` (release is 0.12.0; the Releases-page instruction, true only after 11; "terminal, editor, API client, and database panels", two of which are deleted; SignPath "cannot be applied for … this one is still private"); `site/index.html:158` against `:246`; `docs/beta/02-recruiting-message.md:10` ("the repo is private, so the user cannot open an issue"); `.superpowers/HANDOFF.md` §1–2, still "0.11.1 is cut"; `IDEAS.md`'s moat list | `docs-writer` / `marketing` | Steps 5, 9 and 11 | **largely closed 2026-09-11 → 2026-09-15.** `README.md` corrected 09-11 (`2f7aaa1`); `02-recruiting-message.md`'s private-repo line and `site/index.html`'s self-contradiction and stale version corrected 09-15. **Closed 2026-09-15, all four items, and the closures were verified by reading the files rather than by being reported: (a)** `site/index.html`'s private-repo paragraph — **gone**; the passage now names the owner's unfiled form as the obstacle, which is the true one (carried in row 5). **(b)** `.superpowers/HANDOFF.md` §1–2 — **corrected by the owner**, who holds that file; it now opens on *"0.14.0 is published — the first build a stranger could download"*. Not edited by this seat. **(c)** `IDEAS.md` — **rewritten 2026-09-15 by `product-director`**, and it was worse than this row recorded. The moat paragraph is deleted and replaced, on two grounds stated in the file: it contradicted the 2026-09-02 standing decision, and it was employer-shaped prose on a public repository. Every checkbox is re-checked against the tree and the mark now means one thing — `[x]` is *shipped and still here*, `[-]` is *built and then deleted, with the release that removed it*. Six entries moved to `[-]`: Work panel, ReleaseBoard, StandupModal, terminal recording, DotnetPanel and the Canvas layout. Three sentences that named deleted things were struck with their replacements beside them: the 1DevTool parity claim (two of its three advantages no longer exist), the MCP paragraph (it named two retired tools; `devdeck_projects` and `devdeck_console_logs` survive), and the "26 database engines — we have three" line (DevDeck now has zero). **(d)** `02-recruiting-message.md`'s delivery-channel sentence — **corrected, and the decision it was waiting on was made**: the file now reads *"The repo is public and v0.14.0 is a signed, downloadable release, so an issue filed there IS reachable — but a stranger will not think to file one … so nothing reaches you unless you ask them to paste it to the channel that recruited them."* That is a channel decision, not a description of the release feed, which is what this row asked for. **The row is closed. The class it existed to kill is not** — see the new (e) |
| **10e** | **The same class, found in code rather than in prose, 2026-09-15** — `CHANGELOG.md`'s 0.14.0 entry says `DbPanel`, `ApiPanel` and `WorkPanel` were deleted "along with their IPC handlers, preload channels, stores". True of Db and Api; **false of Work**: `src/main/work.ts` still ships, `src/main/index.ts:1148-1151` still registers four `work:*` handlers, and `src/preload/index.ts:834` still exposes them — with **no renderer caller anywhere**. It is a reachable main-process channel holding safeStorage-encrypted Jira/Azure credentials and an opt-in insecure-TLS toggle, behind no UI | `docs-writer` (the sentence) / `technical-director` (the module) | Nothing on this list | **the sentence is wrong today and is corrected today; the module waits.** Recorded in `IDEAS.md` under the deleted Work entry. **I am not ordering the deletion before the beta** — it is inert, nobody is installed anywhere, and a sixth wave of pre-beta engineering is exactly what this file has now failed to prevent twice. It is first on the list for the tidy after the fifth recorded session. Note what it says about the register: a changelog can assert a deletion that did not happen, and nothing in this repo checks that a retired feature's IPC surface went with it |
...
| **12** | **Lead with the cockpit, demote the suite** — the published claim must match the repo's own description and survive D1 | `marketing` / `product-director` | Step 10 being coherent; the recruiting message being true | **ordered 2026-09-08, and now down to one bullet.** `README.md:3`, this file's line 3, GitHub's repo description and — as of 2026-09-15 — `site/index.html`'s hero are cockpit-first, the hero having gone further than ordered by naming Herdr and Pane in the claim itself. ~~**Outstanding: `PRODUCT.md:14`**, which still promises "terminals, editor, **API client, database client, task board**" snapping to a project, and `:62`, which repeats it.~~ **Both corrected 2026-09-15**, verified: `:14` now reads "terminals, editor, browser" and `:62` says "editor, browser, agent supervision". **The bullet has moved rather than closed, and the two survivors are smaller and of different kinds.** **(a)** `PRODUCT.md:61` still tells a reader that "HTTP client and database drivers run in the main process (`mssql`, `mysql2`, `pg`, `node-sqlite3-wasm`)" — **none of those four is a dependency any more**; `package.json` lists five, and the drivers left with the panel in 0.14.0. A stale *tech-stack* line is a milder lie than a stale *value-proposition* line, which is why it outlived it. **(b)** `PRODUCT.md:68` still says *"'all-in-one' is the vision"* — a claim this file ruled **Not real** on 2026-08-25 and forbids as a published claim. It survives in a risks bullet, which is the one place nobody re-reads. `marketing` owns both; neither gates step 9 |
...
2. ~~**Step 10's three survivors and step 12's one bullet.**~~ **Done, all of it,
   on 2026-09-15 — later the same day this list was written.** The site's
   private-repo paragraph, `PRODUCT.md:14`/`:62`, `.superpowers/HANDOFF.md` §1–2
   and the recruiting message's delivery-channel sentence were corrected by
   their owners; `IDEAS.md` was rewritten by this seat. **What replaces this item
   is two lines and one sentence, and it does not need a sitting:**
   `PRODUCT.md:61`'s four deleted database drivers, `PRODUCT.md:68`'s
   "'all-in-one' is the vision", and `CHANGELOG.md` 0.14.0's claim that the Work
   panel's IPC handlers went with it (row 10e). `marketing` + `docs-writer`,
   under an hour, **still in parallel with 1 and still never in front of it.**
   **Note what this item's history is evidence of, because it is the useful
   part:** a document-truth backlog that stood for four days closed in one, the
   day after it was itemised precisely. It was never the size of the work.
