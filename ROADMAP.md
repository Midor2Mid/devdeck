...
- **Coordinated dependency bump** — **SHIPPED 2026-08-24.** `npm audit` is now **0** (was 25: 2 critical, 19 high). Electron 33 -> 43.4.1, Vite 5 -> 7.3.6, electron-vite 3 -> 5, vitest 2 -> 4, @vitejs/plugin-react 4 -> 5.2, electron-builder 25 -> 26.15.3, then `npm audit fix` (no `--force`) for the in-range remainder. Verified: typecheck clean, 985 tests, `npm run build` clean, and `npm run verify:terminal` 14/14 driving the built app - which is what proves the pty native module still loads under Electron 43, the risk that would have sunk this. **Not verified: packaging.** `electron-builder` invokes `powershell.exe` to enumerate code-signing certs and PowerShell cannot execute in the agent environment (exit 3221226505, the same wall that made `worktree.ps1` unrunnable); builder 26 also failed earlier, at its node-module collector, with the same exit code, so whether that is the environment or a builder-26 bug is unsettled. Run `npm run package:dir` in a working shell to close it; if the collector crash persists there, pin `electron-builder@^25.1.8` - its 12 advisories are build-time only, since it never ships. Original triage, kept for the record:
