...
terminal and editor work without any of them. To check what you have, open
PowerShell and type the command name (`claude`, `codex`, `gemini`) — if it
isn't recognized, install that CLI first. DevDeck does its own version of this
same check on launch and tells you plainly what it found: see "The first five
minutes" below.
...
one line about what it does, and a single button, **Open folder…**. Below it:
which agent CLIs DevDeck expects, and a line reporting what it actually found
on your PATH just now — `Found on your PATH: claude, codex, gemini.`, or, if
none were found, that they weren't in PowerShell's PATH (the one DevDeck
reads — they still run from Git Bash, WSL, or an alias, but a launch that does
nothing is usually this). That sentence renders nothing until the check has
actually run — it never guesses.

1. Click **Open folder…** and pick a folder in the normal Windows dialog.
   DevDeck opens it and lands on **Mission**, its overview screen (agent
   sessions, uncommitted changes across your projects, listening ports on
   this machine).
2. Switch to **Terminal** — the second key in the deck along the bottom of
   the window, or `Ctrl+2`. The deck is four keys now: **Mission · Terminal ·
   Browser · Editor**. With no terminals yet, Terminal shows **New
   terminal** plus one launch card per configured agent (**Claude**, **Claude
   Opus**, **Claude (no permission prompts)**, **Codex**, **Gemini** by
   default).
3. Click **New terminal** (or press `Ctrl+Shift+T`) for a plain shell, or
   ...
   view.
4. Once something is running, the deck bar tells you who needs you: a
   **⚑ N wants you** control appears next to the view keys the moment any
   agent session is asking a question, has finished a turn, or has gone
   quiet mid-task — it's absent when nothing does, and clicking it (or
...
By default DevDeck reads the terminal itself to work out who needs you —
output, silence, the bell. If your agent CLI supports hooks, it can tell
DevDeck directly instead; see `docs/attention-hooks.md` for how to wire that
up. Everything above works without doing that.
...
made the cert — see "Get DevDeck" above for what that's worth to anyone else):
...
Native deps (`@lydell/node-pty`, `@xterm/xterm`) are unpacked from the asar
archive automatically (see the `build` field in `package.json`).

## Checking a change actually works

```bash
npm test                  # ~1,900 unit tests
...
- **⚑ wants you** — a control on the deck bar that appears only when at least one agent session needs you (asking a question, stalled, or finished a turn), names how many, and jumps to whichever has waited longest (`Ctrl+Shift+J`). The same count sits on the Windows taskbar icon when DevDeck isn't focused. By default it's read from the terminal (output, silence, the bell); an agent CLI that supports hooks can report its state directly instead — see `docs/attention-hooks.md`.
- **Desktop notifications** — Settings → Notifications; a session that needs you raises a real Windows toast, from the main process, even when DevDeck is minimized.
...
- **Agent awareness** — every agent session (across all projects) sits in its project's deck strip with a live status: idle, working, waiting, needing attention, or not running. Inferred from output activity and the terminal bell by default, or read directly from a hook when one is wired (`docs/attention-hooks.md`) — no fragile output parsing either way.
...
- **Diagnostics** — a capped, deduped, redacted crash log with a *Copy diagnostics* button on crash cards and in Settings → About, for handing someone the actual reason something broke. Nothing is sent anywhere; it goes to your clipboard, and the button tells you so. See `SECURITY.md` for exactly what is and isn't redacted.
...
> **Security:** a remote terminal can run commands on this machine. Keep the token private, prefer Tailscale over any public tunnel, and disable Remote when you don't need it. Both the remote server and the local MCP server ship off by default — see `SECURITY.md` for the full policy, including how to report a vulnerability.
...
| Shortcut | Action |
|---|---|
| `Ctrl+1` … `Ctrl+4` | Switch view (Mission … Editor) |
...
└── renderer/    # React UI — workspace shell, deck, terminal/editor/browser panels
```
